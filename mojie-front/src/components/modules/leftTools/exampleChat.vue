...
<!--                <div class="example_box" v-for="cUrl in getChildList(item.image_path_res)" :key="item.id+cUrl">-->
                <div class="example_box" >
                  <div class="img-box flex items-center justify-center">
                    <img v-draw-start @click="openExample(item)" class="example_image"
                          v-lazy="getChildList(item.image_path_res)+'?x-tos-process=image/resize,w_200/quality,Q_50/format,webp'" :alt="item.text"/>
                  </div>
                  <div class="example_text line1 text-center">{{ item.text }}</div>
                </div>
            </template>
...
<script setup>
import {defineEmits, reactive, defineProps, ref, onMounted, nextTick} from 'vue'
import {queryCategory, queryExamples, queryTag} from "@/api/example.js"
...
const openExample = (item) => {
  emitter.emit('open-example',item)
}
</script>
