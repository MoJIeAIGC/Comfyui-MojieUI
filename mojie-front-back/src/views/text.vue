<template>
    <div>
      <el-table :data="assetsData" style="width: 100%">
        ...
        <el-table-column prop="name" label="名称">
          <!-- <template #default="{ row }">
            <el-input
              v-if="editingIndex === row.id"
              v-model="row.name"
              @blur="handleTagBlur(row)"
            />
            <span v-else @dblclick="handleTagEdit(row)">{{ row.name }}</span>
          </template> -->
        </el-table-column>
        ...
        <el-table-column prop="text" label="文本">
          <template #default="{ row }">
            <el-input
              ...
              @blur="handleBlur(row)"
            />
            <span v-else @dblclick="handleEdit(row)">{{ row.text }}</span>
          </template>
        </el-table-column>
      ...
      <div  >
        ...
        <el-button type="info" @click="resetTagData">全部标签</el-button>
        <el-button style="margin-left: 630px;" type="primary" @click="showAddTagDialog1 = true">新增标签</el-button>
      ...
      <el-dialog v-model="showAddTagDialog1" title="新增标签" style="width: 50%;">
        ...
        <el-form-item label="种类">
          ...
          <el-select v-model="selectedTagForAdd" placeholder="请选择分类" >
            <el-option
  ...
  import { gettag, edittag, addTag,getrealtag ,editrealtag,addrealTag,delrealtag,deltag,getsmall} from '@/api/templateApi'; // 导入 templateApi 相关方法
...
const viewTag = async (row) => {
  const tagResult2 = await getsmall(row.id);
...
const handleDelete = async (index: number, row: any) => {
  try {
    ...
    // 重新获取数据以更新列表
    // 获取标签数据
    const tagResult = await gettag(1);
...
const handleDeletereal = async (index: number, row: any) => {
  try {
    ...
    // 获取标签数据
      ...
      const tagResult2 = await getsmall(row.tag_id);
      if (tagResult2.code === 200) {
        realtagData.value = tagResult2.data;
      } else {
        errorMessage.value = tagResult2.message || '获取标签数据失败';
...
const resetTagData = async () => {
    const tagResult2 = await getrealtag();
    if (tagResult2.code === 200) {
      realtagData.value = tagResult2.data;
    } else {
      errorMessage.value = tagResult2.message || '获取标签数据失败';
  ...
  onMounted(async () => {
    try {
      ...
      const tagResult2 = await getsmall(20);
      if (tagResult2.code === 200) {
        realtagData.value = tagResult2.data;
      } else {
        errorMessage.value = tagResult2.message || '获取标签数据失败';
  ...
  const submitTagEdit = async (row) => {
    ...
    try {
      ...
      // 重新获取标签数据以更新页面
      const tagResult = await getsmall(row.tag_id);
      if (tagResult.code === 200) {
  ...
  const submitNewTag = async () => {
    try {
      const newTagResult = await addTag({ description: newTagForm.value.description,temp:1 });
 ...
 const submitNewTag1 = async () => {
    try {
      const newTagResult = await addrealTag({ name: newTagForm1.value.name, cateid: selectedTagForAdd.value });
      if (newTagResult.code === 200) {
        // 重新获取标签数据以更新页面
        const tagResult = await getsmall(selectedTagForAdd.value);
        if (tagResult.code === 200) {
          realtagData.value = tagResult.data;
          selectedTagForAdd.value = null;
        } else {
          errorMessage.value = tagResult.message || '更新标签数据失败';
        }
