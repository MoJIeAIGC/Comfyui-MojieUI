...
const generationMethods = [
  ...
  { value: 'flex综合模型', label: 'flex综合模型' },
  { value: '奇画3.0', label: '奇画3.0' },
];
...
const fetchData = async (tagId = 0, cateid = 0) => {
  try {
    const response = await gettemplates(tagId, cateid, currentPage.value, pageSize.value);
...
<template>
  <div class="example-view">
    ...
    <div>
      ...
      <el-table-column prop="text" label="提示词" />
      <!-- <el-table-column prop="title" label="标题" /> -->
      ...
      <el-table-column  label="遮罩">
        <template #default="scope">
          <div style="display: flex; flex-wrap: wrap; gap: 5px;">
            <!-- 修改为添加条件判断 -->
            <div v-for="(path, index) in scope.row.mask ? scope.row.mask.split(',') : []" :key="index">
              <img 
                v-if="path"
                :src="path" 
                style="width: 50px; height: 50px; object-fit: cover;"
